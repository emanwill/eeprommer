...
const WRITE = async (address, byte, cmd) => {
  ...
  const writeRequest = manager.createSendExpectAck();
  await writeRequest(SerialMsgFactory.write(addr, bt));

  const readRequest = manager.createSendExpectData();
  const buf = await readRequest(SerialMsgFactory.read(addr));
